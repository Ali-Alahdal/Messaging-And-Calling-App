...
function Message(props) {
    ...
    return (
        // props.left ?
        <div ref={props.lastMessage} className={"flex flex-col p-3 bg-[var(--btn-l)] text-white rounded-2xl gap-1 h-max  my-2 w-max max-w-[500px] max-h-auto overflow-x-hidden " + props.style} >
            {props.username && <span className="text-xs text-gray-300 font-bold">{props.username}</span>}
            <div className="flex gap-3 items-end">
                <span className="break-all">{props.content}</span>
                <span className="text-[9px] text-[#b8b8b8] whitespace-nowrap">{formatedTime}</span>
            </div>
